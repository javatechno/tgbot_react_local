...
export default class NewsList extends React.Component {
    ...
    refreshListData() {
        const that = this;
        console.log("current weekFilter value = " + JSON.stringify(this.props.weekFilter))
        axios.get(
            'http://localhost:8080/news/list',
            {
                ...
                params: {
                    weekFilter: this.state.weekFilter,
    ...
    componentDidMount() {
        ...
        axios.get(
            ...
            this.props.weekFilter,
            {
                ...
                params: {
                    weekFilter: this.state.weekFilter,
                    allianceFilter: this.stateAllianceFilter
                }
            }
        )
            .then(function (response) {
    ...
    handleFilterChange = async (
        ...
        console.log("Что сетим: " + JSON.stringify(value.props.value))
        await this.setState({weekFilter: value.props.value})
        console.log(JSON.stringify(this.state.weekFilter))
        this.refreshListData()
    }

    handleAllianceFilterChange = async (
        event: React.MouseEvent<HTMLDivElement, MouseEvent>,
        value: number,
    ) => {
    ...
    renderItems = () => {
        ...
        const filteredObjects = (arr) => {
            ...
            console.log(JSON.stringify(arr.filter(item => item['round'] === this.state.weekFilter)))
            return arr.filter(item => item['round'] === this.state.weekFilter) // Filter when type is other than "all"
        }
            ...
            <Box sx={{width: '100%', overflowY: 'scroll'}}>
                <Grid container spacing={2}>
                    <Grid item xs={2} sx={{maxHeight: '92vh', overflowY: 'scroll'}}>
                        <Select label={'Фильтр недели'} multiline={true} select={true} SelectProps={{native: true}}
                                sx={{mb: 3, whiteSpace: "initial"}} fullWidth={true}
                                onChange={this.handleFilterChange}
                                defaultValue={levelz[0].value}>
                            {levelz.map((option) => (
                                <MenuItem key={option.value} value={option.value} label={'Фильтр недели'}>
                                    {option.label}
                                </MenuItem>
                            ))}
                        </Select>
                        <Select label={alliances[0]['label']} multiline={true} select={true}
                                ...
                                sx={{mb: 3, whiteSpace: "initial"}} fullWidth={true}
                                onChange={this.handleAllianceFilterChange}
                    ...
                    <Grid item xs={9}>
                        <NewsForm changeStateOnButton={this.refreshListData.bind(this)}
